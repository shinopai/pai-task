<template>
  <div class="py-5 h-screen bg-gray-300 px-2 relative">
    <h2 class="text-left mb-5 font-bold pl-2">Do<span class="float-right pr-2">{{ allProjectTasksDo.length }}</span></h2>
    <div class="max-w-md mx-auto bg-gray-100 shadow-lg rounded-lg overflow-hidden md:max-w-lg">
        <div class="md:flex">
            <div class="w-full p-4">
                <ul>
                    <li v-if="allProjectTasksDo.length == 0">No tasks yet</li>
                    <li v-else v-for="task in allProjectTasksDo" :key="task" class="flex justify-between items-center bg-white mt-2 p-2 hover:shadow-lg rounded cursor-pointer transition">
...
export default {
  ...
  setup(props) {
    const allProjectTasksDo = ref([])

    // get all tasks that state is do and project is props value
    const getAllProjectTasksDo = async () => {
        await Axios.get('/api/projects/' + props.projectId + '/tasks/do')
                   .then( response => {
                       allProjectTasksDo.value = response.data
